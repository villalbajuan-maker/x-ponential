# Equipo

Documento vivo para describir quienes somos, como nos complementamos y como queremos trabajar.

Nada en este documento debe entenderse como definitivo. Es una primera lectura de aportes, inclinaciones y responsabilidades posibles para activar conversacion.

## Integrantes

| Integrante | Lectura inicial de aporte | Posible foco | Pendiente por validar |
| --- | --- | --- | --- |
| Juan | Liderazgo de servicio, lectura comercial, relacionamiento estrategico, vision de producto, experiencia intergeneracional y capacidad para conectar oportunidades con clientes, capital y rutas de escalamiento | Direccion, estrategia comercial, inversionistas, producto, escalamiento, apertura de mercado y estructuracion de oportunidades | Alcance concreto del rol, limites de decision, nivel de representacion externa y posibles comisionamientos definidos por el equipo |
| Ivan | Senior developer con criterio de arquitectura, experiencia de negocio, liderazgo tecnico y capacidad para sentar bases solidas | Fundacion tecnica, backend, arquitectura, calidad y soporte de plataforma | Disponibilidad, tipo de liderazgo tecnico y primeras responsabilidades |
| Camilo | Matematico, ciencia de datos, conexion entre personas, convocatoria, vision y direccion | Datos, analitica, estrategia, relacionamiento y activacion del equipo | Foco entre producto, comercial, datos y coordinacion |
| David | Developer full stack con ejecucion fuerte y experiencia reciente en ciberseguridad y prevencion de fraude | Ejecucion tecnica, seguridad, integraciones, producto full stack | Primeras tareas tecnicas y areas donde quiere profundizar |
| Alejo | Perfil senior joven con empresa propia y experiencia tecnica por precisar en esta etapa | Pendiente por perfilar con el equipo | Fortalezas, disponibilidad, intereses y rol inicial |

## Lectura de complementariedad

El equipo combina cinco tipos de energia que conviene cuidar:

- Direccion y relacionamiento.
- Fundacion tecnica.
- Datos, vision y conexion.
- Ejecucion full stack y seguridad.
- Experiencia emprendedora y capacidades por perfilar.

La meta no es encasillar a nadie. La meta es encontrar una distribucion inicial que haga visible el aporte de cada integrante y permita empezar a trabajar.

## Roles vivos

Los roles no son titulos fijos. Son responsabilidades que pueden moverse segun la etapa del proyecto.

En una etapa posterior, el equipo debera definir responsabilidades delegadas y limites de decision por area. La intencion es que no todo tenga que ser consultado en pleno, sino que cada integrante pueda actuar con criterio dentro de un marco acordado, especialmente cuando el equipo haya definido comisionamientos, encargos o ambitos de confianza especificos.

- Facilitacion y liderazgo de servicio.
- Estrategia y priorizacion.
- Producto y descubrimiento de oportunidades.
- Arquitectura y tecnologia.
- Operaciones, finanzas y seguimiento.
- Go-to-market, alianzas y ventas.
- Datos, analitica e inteligencia aplicada.
- Seguridad, riesgo y confianza digital.

## Expectativas

- Llegar preparados a las conversaciones importantes.
- Documentar acuerdos y compromisos.
- Cuidar el tiempo del equipo.
- Hacer preguntas temprano cuando algo no este claro.
- Pedir ayuda antes de bloquearse en silencio.
- Celebrar avance real, incluso cuando sea pequeno.

## Participacion esperada en el kickoff

Cada integrante deberia poder responder:

- Que parte de esta lectura si me representa?
- Que parte debo corregir?
- Que puedo aportar en los proximos 30 dias?
- Que necesito del equipo para aportar mejor?
- Que oportunidad me gustaria explorar primero?

## Preguntas para completar

- Que sabe hacer excepcionalmente bien cada integrante?
- Que tipo de problema le energiza a cada uno?
- Que restricciones reales de tiempo tiene el equipo?
- Que nivel de riesgo estamos dispuestos a asumir?
- Que significa para nosotros que una iniciativa valga la pena?
